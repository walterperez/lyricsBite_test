...
import { musixMatchApiKey, audioCrobblerApiKey } from "../../constants/api";
import defImage from "../../static/imageDef.png";
import "./songPage.css";
...
const SongPage = props => {
  ...
  useEffect(() => {
    ...
    fetch(
      `${musixMatch}/ws/1.1/track.lyrics.get?track_id=${trackId}&apikey=${musixMatchApiKey}`
    )
      ...
      .then(data => {
        ...
        return fetch(
          `${musixMatch}/ws/1.1/track.search?q_track=${songTrack}&apikey=${musixMatchApiKey}`
        )
          .then(res => res.json())
          .then(data => {
            ...
            return fetch(
              `${musixMatch}/ws/1.1/album.tracks.get?album_id=${idAlbum}&apikey=${musixMatchApiKey}`
            )
              .then(res => res.json())
              .then(data => {
  ...
  useEffect(() => {
    ...
    fetch(
      `${lastfm}/2.0/?method=album.search&album=${album}&api_key=${audioCrobblerApiKey}&format=json`,
      { signal: signal }
    )
      .then(res => res.json())
      .then(data => {
  ...
  const getAlbumTracks = (idTrack, idAlbum) => {
    fetch(
      `${musixMatch}/ws/1.1/track.lyrics.get?track_id=${idTrack}&apikey=${musixMatchApiKey}`
    )
      .then(res => res.json())
      .then(data => {
